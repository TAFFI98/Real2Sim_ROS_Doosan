import cv2
import numpy as np
# reworked from https://github.com/Schukuratsu/Python-cv2-fast-thinning-algorithm/tree/master
...
def get_white_pixel_positions(binary_image):
    # Find the positions of white pixels in the binary image
    white_pixel_positions = cv2.findNonZero(binary_image)

    # If there are no white pixels, return an empty list
    if white_pixel_positions is None:
        return []

    # Reshape the array to a 2D array containing (row, column) pairs
    white_pixel_positions = white_pixel_positions.reshape(-1, 2)
